...
const dragMimeTypes = ['application/x-radio-card-id', 'text/plain'];

function eventHasCardData(ev){
  const types = Array.from(ev?.dataTransfer?.types || []);
  return dragMimeTypes.some(type => types.includes(type));
}
...
document.addEventListener('drop', (e) => {
  ...
  if (!e.target.closest('.result-card')){
    toast('برای اضافه کردن کارت، بکش و بنداز داخل «میز کار».');
  }
}, {passive:false});

function attachWorkspaceDrag(card, grip, id){
  if (!card || !grip || !id) return;
  const disable = () => { card.draggable = false; };
  const enable = () => { card.draggable = true; };

  disable();

  grip.addEventListener('mousedown', enable);
  grip.addEventListener('mouseup', () => setTimeout(disable, 0));
  grip.addEventListener('mouseleave', () => setTimeout(disable, 0));
  grip.addEventListener('touchstart', enable, {passive:true});
  grip.addEventListener('touchend', () => setTimeout(disable, 0), {passive:true});
  grip.addEventListener('touchcancel', () => setTimeout(disable, 0), {passive:true});

  card.addEventListener('dragstart', (e) => {
    const ws = window.__workspace;
    if (!ws || !e.dataTransfer){
      e.preventDefault();
      disable();
      return;
    }
    ...
    e.dataTransfer.setData('application/x-radio-card-id', id);
...
function render(){
  ...
  if(results.length===0){
    ...
    results.forEach((f, k)=>{
      ...
      const cardId = `${currentProject || 'default'}::${orig}`;
      ...
      handle.type = 'button';
      handle.className = 'drag-grip';
      handle.setAttribute('aria-label', 'Drag card to workspace');
      handle.title = 'Drag';
      handle.innerHTML = '';
      ...
      path.className = 'path card-title';
      ...
      attachWorkspaceDrag(card, handle, cardId);
    });
  }
...
document.getElementById('filterUsedBtn').onclick = ()=>{
  ...
  renderAddTagUI();
  if (window.__workspace && typeof window.__workspace.refresh === 'function') {
    window.__workspace.refresh();
  }
}
